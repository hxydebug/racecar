...
int main(int argc, char** argv)
{
    ...
    std::string port;
    if (n.hasParam("port_imu"))
    n.getParam("port_imu", port);
    else
    {
        ROS_ERROR("%s: must provide a port_imu", name.c_str());
        return -1;
    }
    ...
    ROS_INFO("Model set to %s", model.c_str());
    ...
    {
        ROS_ERROR("%s:imu must provide a baudrate", name.c_str());
        return -1;
    }



    ROS_INFO("Baudrate set to %d", baud);
    ...
    {
        ROS_ERROR("%s:imu Failed to open port %s with error %s",
    ...
    {
        ROS_ERROR("%s:imu failed to open serial port %s",
    ...
    if(fd_ < 0)
    {
        ROS_ERROR("imu Port Error!: %s", path);
        return -1;
    }
    ...
    double vyaw_sum = 0;
    double vyaw_bias = 0;
    pub = n.advertise<sensor_msgs::Imu>("/imu/imu_data", 1);
    ...
    while (n.ok())
    {
        ...
        for(kk = 0; kk < data_length - 1; ++kk)
        {
            ...
            if(model == "art_imu_02a" && data_raw[kk] == 0xA5 && data_raw[kk + 1] == 0x5A)
            {
                ...
                if (check != check_true)
                {
                    printf("imu check error,please wait\n");
                    continue;
                }
