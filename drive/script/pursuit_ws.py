...
class Pursuit:
    ...
    def controlLoop(self):
        ...
        self.speed_kp = self.move_speed_pid[self.speed_v][0];
        self.speed_ki = self.move_speed_pid[self.speed_v][1];
        self.speed_kd = self.move_speed_pid[self.speed_v][2]
        pwm = self.move_speed_pid[self.speed_v][3]

        self.Lfw = self.move_steer_pid[v][3]
        self.steer_kp = self.move_steer_pid[v][0];
        self.steer_ki = self.move_steer_pid[v][1];
        self.steer_kd = self.move_steer_pid[v][2]
    ...
    def isWayPtAwayFromLfwDist(self, waypt, carPose_pos,k=1):
        dx = waypt.x - carPose_pos.x
        dy = waypt.y - carPose_pos.y
        distance = math.sqrt(dx * dx + dy * dy)

        if distance < k*self.Lfw:
            return False
        else:
            return True
    ...
    def getYawFromPose(self, carPose):
        (r, p, y) = tf.transformations.euler_from_quaternion(
            [carPose.orientation.x, carPose.orientation.y, carPose.orientation.z, carPose.orientation.w])
        return y
    ...
    def getSteeringAngle(self, eta):
        steeringAnge = -math.atan2(self.L * math.sin(eta), (self.Lfw / 2 + self.lfw * math.cos(eta))) * (
                    180.0 / math.pi)
        return steeringAnge
    ...
    def get_odom_car2WayPtVec(self, carPose):
        ...
        #                     print each_poseStamped
        #                     self.foundForwardPt = True
        #                     break
        #         except Exception, e:
        #             print e
        #             print 'def get_odom_car2WayPtVec(self,carPose):  Exception!!!'
        forwardPts = [Point(), Point(), Point(),Point()]
        forwardPt = Point()
        forwardPt_k = [0.5, 0.8, 1 , 1.2]
        forwardPt_weight = [0.5, 0.8, 1 ,0.5]
        forwardPt_id = 0
        self.foundForwardPt = False
        ...
        if not self.goal_reached:
            for each_poseStamped in self.map_path.poses:
                ...
                try:
                    ...
                    if _isForwardWayPt:
                        _isWayPtAwayFromLfwDist = self.isWayPtAwayFromLfwDist(odom_path_wayPt, carPose_pos,
                                                                              forwardPt_k[forwardPt_id])
                        if _isWayPtAwayFromLfwDist:
                            forwardPts[forwardPt_id] = odom_path_wayPt
                            # print forwardPt_id, odom_path_wayPt
                            if forwardPt_id == 3:
                                k_sum = sum(forwardPt_weight)
                                for id in range(len(forwardPt_k)):
                                    forwardPt.x+=forwardPts[id].x*forwardPt_weight[id]/k_sum
                                    forwardPt.y+=forwardPts[id].y*forwardPt_weight[id]/k_sum
                                # forwardPt.x = (forwardPts[0].x * forwardPt_weight[0] + forwardPts[1].x * forwardPt_weight[1] +
                                #                forwardPts[2].x * forwardPt_weight[2]) / k_sum
                                # forwardPt.y = (forwardPts[0].y * forwardPt_weight[0] + forwardPts[1].y * forwardPt_weight[1] +
                                #                forwardPts[2].y * forwardPt_weight[2]) / k_sum
                                forwardPt.z = forwardPts[2].z
                                self.foundForwardPt = True
                                # print forwardPt
                                break
                            else:
        ...
        odom_car2WayPtVec.x = math.cos(carPose_yaw) * (forwardPt.x - carPose_pos.x) + math.sin(carPose_yaw) * (
                    forwardPt.y - carPose_pos.y)
        odom_car2WayPtVec.y = -math.sin(carPose_yaw) * (forwardPt.x - carPose_pos.x) + math.cos(carPose_yaw) * (
                    forwardPt.y - carPose_pos.y)

        odom_car2WayPtVec2=Point()
        odom_car2WayPtVec2.x = math.cos(carPose_yaw) * (forwardPts[2].x - carPose_pos.x) + math.sin(carPose_yaw) * (
                    forwardPts[2].y - carPose_pos.y)
        odom_car2WayPtVec2.y = -math.sin(carPose_yaw) * (forwardPts[2].x - carPose_pos.x) + math.cos(carPose_yaw) * (
                    forwardPts[2].y - carPose_pos.y)
        print "get:"
        print odom_car2WayPtVec
        print "real:"
        print odom_car2WayPtVec2
